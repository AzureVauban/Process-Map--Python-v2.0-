...
#!        self.assertEqual(teststring, 'this_is_a_test_string')  # assert that the string is formatted correctly
    ...
    def test_pandacsvparsesearch(self) -> dict:
        """
    ...
    def emplacelink(self, parent: Node, csvrow: list) -> bool:
        ...
        if parent.treekey == csvrow[0] and csvrow[3] != 'None' and csvrow[3] == parent.ingredient and csvrow[7] > 0:
            ...
            # create a node from the list
            child: Node = Node(
                csvrow[1], parent, csvrow[5], csvrow[6], csvrow[6], False, False, csvrow[0])
            return True
        else:
            return False
    ...
    def test_headnodecreation(self):
        ...
        if foundheadnodes == {-1: None} or not os.path.exists(path=TESTFILENAME):
            ...
            # assert that the population of the tree is equal to the population of the mock tree
            self.assertEqual(nodecount, 10)
    ...
    def istreesame(self, presetingredienttree: Node, csvsourcedtree: Node) -> bool:
        ...
        print('\x1B[35m', presetingredienttree.ingredient+'\x1B[37m',
              '\x1B[31m'+csvsourcedtree.ingredient+'\x1B[37m')  # debug
...
# ?            return False
            ...
            for index, node in enumerate(presetingredienttree.children.items()):
                ...
                return self.istreesame(list(presetingredienttree.children.items())[index][1], list(csvsourcedtree.children.items())[index][1])
    ...
    def test_createdtreeissame(self):
        ...
        self.assertTrue(self.istreesame(industrial_battery,
                        testhead), 'the ingredent trees are not the same')
...
if __name__ == '__main__':
    ...
    # call the function to test the csv parsing and search method
    print('\nNode:', yellow, '('+yellow.treekey+')')
